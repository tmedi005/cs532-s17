...
def read(feed,classifier):
  ...
  string1 = '90 manual classifications\n\n'
  dividline = "_" * 100 + "\n"
  print string1
  savefile.write(string1)
  string2 = "{:<60}{:<12}{:<10}{}".format("Title", "Prediction", "Actual", "\n")
  print string2
  savefile.write(string2)
  savefile.write(dividline)

  for entry in f['entries'][0:90]:
    num=num +1
  ...
  savefile.write(dividline)
  num=90
  string4 = '\n\n10 automatic classifications\n\n'
  print string4
  savefile.write(string4)
  savefile.write(string2)
  savefile.write(dividline)
  for entry in f['entries'][90:100]:
    num=num+1
    title=entry['title'].encode('utf-8').replace("'","")
    print 'Title:     '+ title
    summary = splitRegexp.sub( "", entry[ "summary" ] )
